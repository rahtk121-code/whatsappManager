...
export function isClosingMessage(text = "") {
  ...
  return /(شكرا|شكر|مشكور|تسلم|يعطيك العافيه|يعطيك العافية|تمام|خلاص|اوكي|ok|thanks|thank you|جزاك الله خير|ما قصرت|ماقصرت|بارك الله فيك)/.test(
    t
  );
}
...
export function findBestProduct(text, products = []) {
  ...
  for (const product of products) {
    ...
    if (t.includes("ايفون") && (name.includes("iphone") || name.includes("ايفون"))) {
      score += 10;
    }

    if (t.includes("سامسونج") && (name.includes("samsung") || name.includes("سامسونج"))) {
      score += 10;
    }
...
export function localReply({ text, products = [], settings }) {
  ...
  if (isClosingMessage(text)) {
    return settings?.aiLanguage === "formal"
      ? "على الرحب والسعة، سعدنا بخدمتك 🌟"
      : "العفو، حياك الله 🌟";
  }
      ...
      messages: [
        {
          role: "system",
          content: `
أنت بائع واتساب محترف جدًا، تتحدث كإنسان وليس كروبوت.
...
وصف المتجر:
${settings?.storeDescription || "متجر إلكتروني"}

سياسة الشحن:
${settings?.shippingPolicy || "غير محددة"}

سياسة الدفع:
${settings?.paymentPolicy || "غير محددة"}

أفضل منتج مطابق:
${
  matchedProduct
    ? `${matchedProduct.name} | السعر ${matchedProduct.price} | المخزون ${matchedProduct.stock}`
    : "لا يوجد"
}
`,
        },
